            ...
            @if (Auth::user() != null)
                <div class="hidden sm:flex sm:items-center sm:ml-6">
                    <x-dropdown align="right" width="48">
                        ...
                        <x-slot name="content">
                            <x-dropdown-link :href="route('users.detail', Auth::user()->username)">
                                {{ __('Profile') }}
                            </x-dropdown-link>
                            @if(Auth::user()->show_friends_in_nav_dropdown === 1)
                                <x-dropdown-link :href="route('users.detail', [Auth::user()->username, 'view' => 'friends'])">
                                    {{ __('Friends') }}
                                </x-dropdown-link>
                            @endif
                            ...
                            @endif
                            @if(Auth::user()->show_favourites_in_nav_dropdown === 1)
                                <x-dropdown-link :href="route('users.detail', [Auth::user()->username, 'view' => 'favourites'])">
                                    {{ __('Favourites') }}
                                </x-dropdown-link>
                            @endif
        ...
        @if (Auth::user() != null)
            ...
            <div class="pt-4 pb-1 border-t border-gray-200 dark:border-gray-700">
                ...
                <div class="mt-3 space-y-1">
                    <x-responsive-nav-link :href="route('users.detail', Auth::user()->username)">
                        {{ __('Profile') }}
                    </x-responsive-nav-link>
                    @if(Auth::user()->show_friends_in_nav_dropdown === 1)
                    ...
                    @endif
                    @if(Auth::user()->show_favourites_in_nav_dropdown === 1)
                        <x-responsive-nav-link :href="route('users.detail', [Auth::user()->username, 'view' => 'favourites'])">
                            {{ __('Favourites') }}
                        </x-responsive-nav-link>
                    @endif
