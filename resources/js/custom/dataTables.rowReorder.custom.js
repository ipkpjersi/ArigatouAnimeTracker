...
$.extend(RowReorder.prototype, {
	...
	_mouseDown: function (e, target) {
		...
		// -------- START: CUSTOM group segment clamp setup for watch_status --------
		var nodes = start.nodes;
		var targetStatus = target.attr('data-watch-status');

		if (targetStatus) {
			// Find the dragged row's position in the nodes array
			var targetEl = target[0];
			var startIdx = $.inArray(targetEl, nodes);

			// Expand to capture only the contiguous block of same watch_status
			var first = startIdx, last = startIdx;
			while (first - 1 >= 0 && $(nodes[first - 1]).attr('data-watch-status') === targetStatus) {
				first--;
			}
			while (last + 1 < nodes.length && $(nodes[last + 1]).attr('data-watch-status') === targetStatus) {
				last++;
			}

			this.s.watchStatusGroup = { first: first, last: last, status: targetStatus };
		} else {
			this.s.watchStatusGroup = null;
		}
		// -------- END: CUSTOM group segment clamp setup --------
	...
	_mouseMove: function (e) {
		...
		// -------- START: CUSTOM clamp insertPoint to watch_status group --------
		if (this.s.watchStatusGroup) {
			var group = this.s.watchStatusGroup;
			// Clamp insertPoint to [first, last+1] to keep within the same watch_status group
			if (insertPoint < group.first) {
				insertPoint = group.first;
			} else if (insertPoint > group.last + 1) {
				insertPoint = group.last + 1;
			}
		}
		// -------- END: CUSTOM clamp --------
	...
	_moveTargetIntoPosition: function (insertPoint) {
		...
		if (this.s.lastInsert === null || this.s.lastInsert !== insertPoint) {
			...
			// --- BEGIN: CUSTOM group-aware insertion ---
			var g = this.s.watchStatusGroup;
			if (g) {
				if (insertPoint === g.first) {
					// pin ABOVE first row in this segment
					this.dom.target.insertBefore(nodes[g.first]);
					insertPlacement = 'before';
				} else if (insertPoint === g.last + 1) {
					// pin BELOW last row in this segment
					this.dom.target.insertAfter(nodes[g.last]);
					insertPlacement = 'after';
				} else {
					// interior movement within the segment
					if (this.s.lastInsert !== null && insertPoint > this.s.lastInsert) {
				...
				// fallback (no group info) - original logic
			...
			// --- END: CUSTOM group-aware insertion ---
